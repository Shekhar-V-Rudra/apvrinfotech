...
@section('content')
<div class="max-w-3xl">
    <h1 class="text-2xl font-semibold text-gray-800 mb-6">Add Career</h1>

    <form method="POST" action="{{ route('admin.careers.store') }}" class="bg-white rounded-lg shadow p-6 space-y-5">
        @csrf

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Job Title</label>
            <input name="job_title" value="{{ old('job_title') }}" class="w-full rounded border-gray-300 focus:ring-indigo-500 focus:border-indigo-500" required />
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="vacancy" value="{{ old('vacancy', 1) }}" min="1" class="w-full rounded border-gray-300 focus:ring-indigo-500 focus:border-indigo-500" required />
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input name="experience" value="{{ old('experience') }}" class="w-full rounded border-gray-300 focus:ring-indigo-500 focus:border-indigo-500" placeholder="e.g., 0-2 Years" />
                @error('experience')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Location</label>
                <input name="location" value="{{ old('location') }}" class="w-full rounded border-gray-300 focus:ring-indigo-500 focus:border-indigo-500" placeholder="e.g., Surat" />
